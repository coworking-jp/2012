...
;(function($) {
	...
	// Update contents of the balloon
	function updateContents($target, $balloon, options) {
		if(options.url) {
			$balloon.load(options.url, function(res, sts, xhr) {
				if(options.ajaxComplete) options.ajaxComplete(res, sts, xhr);
				adjustBalloon($target, $balloon, options);
			});
		} else {
			var contents = options.contents || $target.attr("title");
			if(contents && contents != $balloon.html()) {
				$balloon.html(contents);
	...
	$.fn.showBalloon = function(options) {
		...
		return this.each(function() {
			...
			if($target.data("balloon")) {
				...
				updateContents($target, $balloon, options);
				adjustBalloon($target, $balloon, options);
			} else {
				$balloon = (options.contents)
					? $("<div>").append(options.contents)
					: $("<div>", {html: $target.attr("title")});
...
/**
 *  TODO:
 *  - Add an option to follow mouse pointer. ("followCursor: true")
 *  - Enable to use function as contents and url.
 *  - Preload images?
**/
